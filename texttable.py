...
class Texttable:
    ...
    def _len_cell(self, cell):
        ...
        # patched -- delete all colors
        #len_cell_line = re.sub("\033\[[^m]*m", "", str(cell))
        #len_cell = len(len_cell_line)
        return maxi
    ...
    def _draw_line(self, line, isheader=False):
        ...
        for i in range(len(line[0])):
            ...
            for cell, width, align in zip(line, self._width, self._align):
                ...
                # patched -- delete all colors
                len_cell_line = re.sub("\033\[[^m]*m", "", cell_line)
                fill = width - len(len_cell_line)
    ...
    def _splitit(self, line, isheader):
        ...
        for cell, width in zip(line, self._width):
            ...
            for c in cell.split('\n'):
                ...
                # use own wrapper function
                lineWrapped = textwrap.wrap(c, width)

                array.extend(lineWrapped)
        ...
        for cell, valign in zip(line_wrapped, self._valign):
            ...
            if isheader:
                valign = "t"
            if valign == "m":
                missing = max_cell_lines - len_cell
                cell[:0] = [""] * (missing / 2)
                cell.extend([""] * (missing / 2 + missing % 2))
            elif valign == "b":
                cell[:0] = [""] * (max_cell_lines - len_cell)
            else:
                cell.extend([""] * (max_cell_lines - len_cell))
        return line_wrapped
